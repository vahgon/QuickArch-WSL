...
pacman -S git base-devel curl man-db openssh gdb wget tree fzf gcc python-virtualenv python-pycurl git unzip libcurl-compat python python-pip python-pipx pyenv rustup nano vim make tmux zip direnv nmap ruby python-pipenv python-requests --noconfirm 
